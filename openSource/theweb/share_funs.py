...
thedb="data.db"#实际数据库文件的储存位置
#以列表形式返回所有的社区名
def readnames():
    cx = sqlite3.connect(thedb)
    cu = cx.cursor()
    sql1='select name from share_table'
...
def readcontent(name):
    cx = sqlite3.connect(thedb)
    cu = cx.cursor()
    sql1 = 'select content from share_table where name=?'
...
def readweb(name):
    cx = sqlite3.connect(thedb)
    cu = cx.cursor()
    sql1 = 'select web from share_table where name=?'
    thelist1=list(cx.execute(sql1,(name,)))
    time=thelist1[0][0]
    cx.close()
    return time
...
def readimg(name,imgname):
    ...
    cu = cx.cursor()
    f=open(imgname,'wb')
    sql1 = 'select img from share_table where name=?'
    thelist1 = list(cx.execute(sql1, (name,)))
